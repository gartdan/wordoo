...
namespace WordCloudGenerator.Web.TagCloud
{
    /// <summary>
    ...
    public class TagCloud
    {
        ...
        public static IEnumerable<Tag> CreateTags(IEnumerable<KeyValuePair<string, int>> weightedTags, TagCloudGenerationRules generationRules)
        {
            ...
            IEnumerable<Tag> cloudTags = from weightedTag in weightedTags
                                         select new Tag
                                         {
                                             ...
                                             NavigateUrl = string.Format(generationRules.TagUrlFormatString, System.Net.WebUtility.UrlEncode(weightedTag.Key)),
        ...
        public override string ToString()
        {
            ...
            div.MergeAttribute("class", "TagCloud");
            foreach(var tag in this.Tags)
            {
                var span = new TagBuilder("span");
                if (!string.IsNullOrEmpty(tag.CssClass)) span.MergeAttribute("class", tag.CssClass);
                if (!string.IsNullOrEmpty(tag.ToolTip)) span.MergeAttribute("title", tag.ToolTip);
                if (tag.HtmlAttributes != null)
                    foreach (KeyValuePair<string, object> attribute in tag.HtmlAttributes)
                        span.MergeAttribute(attribute.Key, Convert.ToString(attribute.Value, CultureInfo.InvariantCulture));

                var a = new TagBuilder("a");
                if(!string.IsNullOrEmpty(tag.NavigateUrl))
                {
                    a.MergeAttribute("href", tag.NavigateUrl);
                    a.InnerHtml.SetContent(tag.Text);
                    span.InnerHtml.AppendHtml(a);
                }
                else
                {
                    span.InnerHtml.Append(tag.Text);
                }
                
                div.InnerHtml.AppendHtml(span);
            }
            var sb = new StringBuilder();
            div.WriteTo(new StringWriter(sb), HtmlEncoder.Default);
            return sb.ToString();
